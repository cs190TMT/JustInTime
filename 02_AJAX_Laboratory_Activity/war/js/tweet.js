...
$(document).ready(function() {
		...
		$.ajax({
			...
			success : function(data, status, jqXHR) {
				if (data.errorList.length == 0) {
					...
					// TODO: (optional) use the errorDisplay to display the msg
					// string there.
					errorDisplay.html(msg);
				}
			},
	...
	$(document).on('click', '#tweetList .btnUpdate', function() {
		// TODO: code jQuery code for updating tweet. hint: refer to your code
		// for deleting tweets.
		$('#errorDisplay').empty();
		$('.updateErrorDisplay').empty();
 ...
 *            success message to display if the transaction is successful.
 */
								...
								.each(
										...
										function(index, value) {
											formattedTweetList += '<hr />'
													...
													+ value.id
													+ '"/>'
													+ '	<input type="hidden" class="createdDate" name="createdDate" value="'
													+ value.createdDate
													+ '"/>'
